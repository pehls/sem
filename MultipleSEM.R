...
multiplesem <- function (fileName, drop_key, path_upload, ano_atual, 
  ...
  data_raw <- read.spss(fileName,
                        to.data.frame=T,
                        header=T,
                        use.value.labels = F,
                        use.missings = T)
  data_raw <- data_raw[which(data_raw$ANO_RESP == ano_atual),]
  data_raw_aux_t2b <- data_raw[which(data_raw$ANO_RESP == ano_atual),]
  
  data_value_labels <- read.spss(fileName,
                                 to.data.frame=T,
                                 header=T,
                                 use.value.labels = T,
                                 use.missings = T)
  data_value_labels <- data_value_labels[which(data_value_labels$ANO_RESP == ano_atual),]
      ...
      if (is.na(nivel)) {
        ...
        if (n_obs < 100) {
          ...
          if(length(diferentes)>0){
            ...
            ###deve, portanto, ṇo estar incluido em soh_val. a variavel diferentes possui ####
          ...
          fileName<-paste("IBMR",paste(nivel_label,"resultados_SEM.xlsx",sep="_"),sep="_")
          ...
          writeWorksheet(exc, model_completo, sheet ='VarFora',header=T,rownames = T, startRow = 1, startCol = 3)
          saveWorkbook(exc)
