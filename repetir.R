  ...
  for(q in n_quebra:length(quebras)){
    ...
    for(lev in n:length(niv)){
      ...
      if ( (count(data_raw[data_raw[,quebra]==nivel,]) < 31 ) || ((count(data_raw[data_raw[,quebra]==nivel,]) < (length(indices_colunas)*10) ))){
        ...
        for(i in 1:5) #%dopar%
          #dados = data[sample(x = 1:nrow(data), size = 500),]
      ...
      # 
        for(i in 1:5) #%dopar% 
          #dados = data[sample(x = 1:nrow(data), size = 500),]
          do.semx10(indice = i, dados = dados,
                    var_labels = var_labels,
                    nivel = nivel,
                    nivel_label = nivel_label,
                    model_completo_aux = model_completo_aux,
      ...
      for(i in 1:5)# %dopar%
        #dados = data[sample(x = 1:nrow(data), size = 500),]
        do.sem400(indice = i, dados = dados,
               var_labels = var_labels,
               nivel = nivel,
               nivel_label = nivel_label,
               model_completo_aux = model_completo_aux,
      ...
      # 
        for(i in 1:5) #%dopar%
          #dados = data[sample(x = 1:nrow(data), size = 500),]
          do.semall(indice = i, dados = dados,
                    var_labels = var_labels,
                    nivel = nivel,
                    nivel_label = nivel_label,
                    model_completo_aux = model_completo_aux,
...
###### Processo de SEM utilizando base completa #####
do.semall <- function(indice, dados, var_labels,
                   nivel, nivel_label, 
  ...
  fileName <- paste(Estudo,
                    paste(nivel_label,
                          paste(indice,
                                paste(hour,"comNall-sem_outliers.xlsx", sep=""),
                                sep="_"),
                          sep="_"),
                    sep="_")  %>%
    stringr::str_replace_all(.,"/","")  %>%
...
###### Processo de SEM utilizando base 500 #####
do.sem500 <- function(indice, dados, var_labels,
                   nivel, nivel_label, 
                   model_completo_aux, mapeamente_vars_removidas,
                   constructs, var_dep_eq, segunda_ordem, var_lonely, 
                   IES) {
...
###### Processo de SEM utilizando base 400 #####
do.sem400 <- function(indice, dados, var_labels,
                   nivel, nivel_label, 
                   model_completo_aux, mapeamente_vars_removidas,
                   constructs, var_dep_eq, segunda_ordem, var_lonely, 
                   IES) {
  ...
  fileName <- paste(Estudo,
                    paste(nivel_label,
                          paste(indice,
                                paste(hour,"comN400-sem_outliers.xlsx", sep=""),
                                sep="_"),
                          sep="_"),
                    sep="_")  %>%
    stringr::str_replace_all(.,"/","")  %>%
...
do.semx10 <- function(indice, dados, var_labels,
                      nivel, nivel_label, 
                      model_completo_aux, mapeamente_vars_removidas,
                      constructs, var_dep_eq, segunda_ordem, var_lonely, 
                      IES) {
  ...
  dados <-detect.outliers(data_raw, variaveis_do_modelo)
  data_raw <- dados
  tamanho <- length((variaveis_do_modelo)) * 10
  data_raw <- (dados[sample(x = 1:nrow(dados), size = tamanho),])
  dados <- data_raw
  # baixar banco de dados sem labels e filtrar conforme a quebra ####
  ...
  fileName <- paste(Estudo,
                    paste(nivel_label,
                          paste(indice,
                                paste(hour,"comNx10-sem_outliers.xlsx", sep=""),
                                sep="_"),
                          sep="_"),
                    sep="_")  %>%
    stringr::str_replace_all(.,"/","")  %>%
